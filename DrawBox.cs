...
namespace TakaGUI
{
	...
	public abstract class DrawBox
	{
		...
		public static void InitializeDefaultServices(Game game, GraphicsDeviceManager gdManager, string skinFileDir)
		{
			var resourceManager = new ResourceManager(game.Content);
			game.Services.AddService(typeof(IResourceManager), resourceManager);

			var debug = new Debug(game);
			game.Services.AddService(typeof(IDebug), debug);
			
			DefaultSkinFile = new SkinFile(resourceManager, skinFileDir);

			var keyboardInput = new KeyboardInput(game);
			game.Services.AddService(typeof(IKeyboardInput), keyboardInput);

			var mouseInput = new MouseInput(game);
			game.Services.AddService(typeof(IMouseInput), mouseInput);

			var cursorManager = new CursorManager(game, DefaultSkinFile);
			game.Services.AddService(typeof(ICursorManager), cursorManager);

			var graphicsManager = new GraphicsManager(game, gdManager);
			game.Services.AddService(typeof(IGraphicsManager), graphicsManager);			
			...
			InitializeServices(game.Services);
		}
